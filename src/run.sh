...
DATASETS=("reddit_chatgpt") # "wikipedia_chatgpt" "wikipedia_cohere" "wikipedia_bloomz" "arxiv_chatgpt" "arxiv_cohere" "arxiv_bloomz" "reddit_chatgpt" "reddit_cohere" "reddit_bloomz"
        ...
        for MODE in "${MODES[@]}"; do
            for DIFF_VECTOR in "${DIFF_VECTORS[@]}"; do
                for PREFIX_FLAG in "${PREFIX[@]}"; do
                    echo "------------------------------------------------"
                    echo "Running Experiment: Dataset=$DATASET, Model=$MODEL, Mode=$MODE, DiffVec=$DIFF_VECTOR, Prefix=$PREFIX_FLAG"
                    echo "------------------------------------------------"
