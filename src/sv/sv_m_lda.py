...
class MLdaSVBase(SVBase):
    """Per-layer PCA manifold + per-layer LDA scorer fit on validation projections."""
    ...
    def _score_with_models(
        ...
        for layer_idx in range(n_layers):
            ...
            if entry["kind"] == "lda":
                model: LinearDiscriminantAnalysis = entry["model"]
    ...
    def run(self, args: Namespace) -> dict[str, object]:
        ...
        lda_models = self._fit_lda_models_by_layer(hidden_states=val_hidden_proj, labels=val_labels)
        val_projection = self._score_with_models(hidden_states=val_hidden_proj, layer_models=lda_models)
        ...
        test_projection = self._score_with_models(hidden_states=test_hidden_proj, layer_models=lda_models)
        ...
        for ood_dataset_name in args.ood:
            ...
            ood_projection = self._score_with_models(hidden_states=ood_hidden_proj, layer_models=lda_models)
            if args.normalize_scores:
