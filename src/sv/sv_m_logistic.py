...
class MLogisticSVBase(SVBase):
    """Per-layer PCA manifold + per-layer Logistic Regression scorer fit on validation projections."""
    ...
    def fit_pca_manifold_by_layer(
        ...
        for layer_idx in range(n_layers):
            ...
            basis = vt[:k].T.astype(np.float32)  # d_model x k
    ...
    @staticmethod
    def _fit_logistic_models_by_layer(
        hidden_states: np.ndarray,
        labels: np.ndarray,
    ) -> list[dict[str, Any]]:
        ...
        for layer_idx in range(n_layers):
            ...
            # Fallback to mean-difference linear scorer if logistic is unstable.
    ...
    def run(self, args: Namespace) -> dict[str, object]:
        ...
        logistic_models = self._fit_logistic_models_by_layer(hidden_states=val_hidden_proj, labels=val_labels)
        val_projection = self._score_with_models(hidden_states=val_hidden_proj, layer_models=logistic_models)

        test_hidden, test_labels = self._collect_hidden_states(data=dataset["test"], token_mode=args.token_mode)
        if args.manifold and projection_state is not None:
            ...
            test_hidden_proj = test_hidden
        test_projection = self._score_with_models(hidden_states=test_hidden_proj, layer_models=logistic_models)
        ...
        for ood_dataset_name in args.ood:
            ...
            if args.manifold and projection_state is not None:
                ...
                ood_hidden_proj = ood_hidden

            ood_projection = self._score_with_models(hidden_states=ood_hidden_proj, layer_models=logistic_models)
            if args.normalize_scores:
        ...
        return {
            ...
            "n_layers": int(val_projection.shape[1]),
