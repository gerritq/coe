...
class DenoiseValSVBase(SVBase):
    """Denoise validation activations per layer, then compute SV from denoised val states."""
    ...
    @staticmethod
    def denoise_hidden_states_by_layer(
        hidden_states: np.ndarray,
    ...
    def run(self, args: Namespace) -> dict[str, object]:
        ...
        if args.manifold:
            ...
            val_hidden_denoised = self.denoise_hidden_states_by_layer(
                hidden_states=val_hidden,
        ...
        # Compute SV from denoised validation activations.
        ...
        # Keep test/OOD activations raw; only val is denoised in this variant.
