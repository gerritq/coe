...
class PCAAlignSVBase(SVBase):
    """Per-layer PCA variant where components are selected by steering alignment."""

    @staticmethod
    def fit_aligned_pca_manifold(
        ...
        for layer_idx in range(n_layers):
            ...
            # PCA basis for this layer (ordered by explained variance)
            _, _, vt = np.linalg.svd(centered, full_matrices=False)
            full_basis = vt.T.astype(np.float32)  # d_model x n_pc

            # Re-rank this layer's PCs by alignment with this layer's raw steering vector
            layer_sv = steering_vectors[layer_idx]  # d_model
            alignment_score = np.abs(layer_sv @ full_basis)  # n_pc
            top_idx = np.argsort(alignment_score)[::-1][:k]
            aligned_basis = full_basis[:, top_idx].astype(np.float32)  # d_model x k

            means[layer_idx] = layer_mean
            components[layer_idx] = aligned_basis
            projected[:, layer_idx, :] = (centered @ aligned_basis).astype(np.float32)
    ...
    def fit_projection_space(
        ...
        val_hidden_proj, manifold_means, manifold_components = self.fit_aligned_pca_manifold(
