...
class CleanTopicSVBase(SVBase):
    ...
    def ablate_hidden_states_with_axes(
        ...
        # Apply the requested ablation exactly: h_ablated = h - r(r^T h), with r normalized.
    ...
    def run(self, args: Namespace) -> dict[str, object]:
        ...
        # Residual detector: compute SV in the ablated space and score in that same space.
        ...
        test_hidden_ablated = self.ablate_hidden_states_with_axes(
        ...
        test_projection = self.compute_projection(
            val_hidden_states=val_hidden_ablated,
            test_hidden_states=test_hidden_ablated,
            steering_vectors=steering_vec,
            args=args,
        )
        ...
        for ood_dataset_name in args.ood:
            ...
            ood_hidden_ablated = self.ablate_hidden_states_with_axes(
            ...
            ood_projection = self.compute_projection(
                val_hidden_states=val_hidden_ablated,
                test_hidden_states=ood_hidden_ablated,
                steering_vectors=steering_vec,
                args=args,
            )
            if args.normalize_scores:
