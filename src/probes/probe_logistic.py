...
class LogisticProbeBase(ProbeBase):
    """Per-layer logistic probes fit on validation activations."""
    ...
    def fit_logistic_models_by_layer(
        ...
        _n_samples, n_layers, _d_model = hidden_states.shape
    ...
    def score_with_models(
        ...
        n_samples, n_layers, _d_model = hidden_states.shape
    ...
    def run(self, args: Namespace) -> dict[str, object]:
        ...
        logistic_models = self.fit_logistic_models_by_layer(hidden_states=val_hidden, labels=val_labels)
        val_scores = self.score_with_models(hidden_states=val_hidden, layer_models=logistic_models)

        test_hidden, test_labels = self._collect_hidden_states(data=dataset["test"], token_mode=args.token_mode)
        test_scores = self.score_with_models(hidden_states=test_hidden, layer_models=logistic_models)
        ...
        for ood_dataset_name in args.ood:
            ...
            ood_scores = self.score_with_models(hidden_states=ood_hidden, layer_models=logistic_models)
        ...
        return {
            ...
            "d_model": int(val_hidden.shape[2]),
            "ood": bool(args.ood),
            "manifold": False,
        }
