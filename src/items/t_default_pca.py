...
def render_table(scores: dict[str, dict[str, float]]) -> str:
    ...
    lines = [
        ...
        r"\midrule",
        _table_row("LLP (No PCA)", [_fmt(scores["default"][dataset]) for dataset in datasets]),
        _table_row("LLP (PCA)", [_fmt(scores["pca"][dataset]) for dataset in datasets]),
        r"\midrule",
        _table_row(
            r"LLP (No PCA) $-$ LLP (PCA)",
