...
OUT_PATH = os.path.join(BASE_DIR, "output", "item", "t_meta_no_pca_comparison.tex")
...
def _probe_auroc(obj: dict) -> float | None:
    auroc = obj.get("test_metrics", {}).get("meta_metrics", {}).get("auroc")
    if auroc is None:
        return None
    auroc = float(auroc)
    if math.isnan(auroc):
...
def collect_probes() -> dict[str, dict[str, float]]:
    scores: dict[str, dict[str, float]] = {"meta": {}, "meta_no_pca": {}}
...
def render_table(scores: dict[str, dict[str, float]]) -> str:
    ...
    lines = [
        ...
        r"\midrule",
        _table_row("CLP (PCA)", [_fmt(scores["meta"][dataset]) for dataset in datasets]),
        _table_row("CLP (No PCA)", [_fmt(scores["meta_no_pca"][dataset]) for dataset in datasets]),
        r"\midrule",
        _table_row(
            r"CLP (PCA) $-$ CLP (No PCA)",
            [_fmt(scores["meta"][dataset] - scores["meta_no_pca"][dataset]) for dataset in datasets],
        ),
        r"\bottomrule",
        r"\end{tabular}",
    ]
