...
SUBSET_LABELS = {
    ...
    "tsm_tst": r"\textbf{TST}",
    "raidModel_cohere_chat": r"\textbf{Cohere}",
    "raidModel_gpt4": r"\textbf{GPT4}",
    "raidModel_llama_chat": r"\textbf{Llama}",
    "raidModel_mistral_chat": r"\textbf{Mistral}",
}

DATASET_ALIASES = {
    "raid_cohere_chat": "raidModel_cohere_chat",
    "raid_gpt4": "raidModel_gpt4",
    "raid_llama_chat": "raidModel_llama_chat",
    "raid_mistral_chat": "raidModel_mistral_chat",
}
...
METHOD_SPECS = [
    ...
    ("LLP", {"kind": "probe", "mode": "pca", "components": 100}),
    ("CLP", {"kind": "probe", "mode": "meta", "components": 100}),
]
    ...
    if spec["kind"] == "probe":
        for filename in sorted(os.listdir(PROBE_DIR)):
            ...
            if args.get("mode") != spec["mode"]:
                continue
            if int(args.get("components", -1)) != int(spec.get("components", -1)):
                continue
...
def main() -> None:
    ...
    out_path = os.path.join(OUT_DIR, "t_ood_pca.tex")
